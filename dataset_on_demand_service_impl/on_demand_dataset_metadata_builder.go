...
import (
	...
	"github.com/NBISweden/bp-dod-sda-gateway/pkg/accession"
...
func buildOnDemandDataset(ctx context.Context, originDatasets map[string]*models.OriginDataset, datasetImages map[string]map[string]struct{}) *models.OnDemandDataset {
	_, span := observability.StartSpan(ctx, "buildOnDemandDataset")
	defer span.End()

	datasetAccession := accession.GenerateAccession(accession.Dataset)

	dodDatasetMetadata := &models.OnDemandDataset{
		...
		DatasetMetadata: models.MetadataFile[*metadata_models.DatasetSet]{
			Accession:   accession.GenerateAccession(accession.File),
			...
			remsEntry.Attributes.Value.SetAttributes[0].Value.Value.StringAttributes = append(
				...
				metadata_models.StringAttribute{
					...
					Value: &metadata_models.NullableString{
						Value: &originDatasetAccession,
					},
				},
			)
			remsEntries[originDataset.RemsWorkflowID] = remsEntry
		} else {
			remsAccession := accession.GenerateAccession(accession.File)
		...
		Dataset: []metadata_models.Dataset{
			{
				...
				Title:                    "On Demand Dataset",
				ShortName:                "On Demand Dataset",
				Description:              nil,
				Version:                  "1.0.0",
