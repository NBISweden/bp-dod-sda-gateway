...
func Init(ctx context.Context) error {
	...
	// Parse the user from the inbox.Token, and sdaApiToken, as these need to match and store for use when triggering ingestion, etc
...
func (dmfh *dodMetadataFileHandler) monitorDatasetMetadataFiles(datasetAccession string, datasetMetadataFiles map[metadata_models.MetadataFileType]string) {
	for {
		if dmfh.ctx.Err() != nil {
			slog.Info("stopping dod metadata file handler - context canceled")
		...
		if done {
			slog.Info("dod metadata file handler - process dataset metadata files finished", "accession", datasetAccession)

			break
		}

		time.Sleep(sdaAPIPollInterval)
	}
}
...
func (dmfh *dodMetadataFileHandler) pollAndProcess(datasetAccession string, datasetMetadataFiles map[metadata_models.MetadataFileType]string) (bool, error) {
	...
	remsMetadata, err := database.GetDatasetOnDemandDatasetRemsMetadata(ctx, datasetAccession)
	if err != nil {
		return false, err
	}
	...
	if err := database.SetDatasetOnDemandDatasetReleased(ctx, datasetAccession); err != nil {
		return false, fmt.Errorf("failed to set dataset on demand dataset as released: %w", err)
	}

	return true, nil
}

func RegisterDatasetOnDemandDataset(ctx context.Context, dodDataset *models.DatasetOnDemandDataset) error {
	ctx, span := observability.Tracer().Start(ctx, "RegisterDatasetOnDemandDataset", trace.WithAttributes(attribute.String("accession", dodDataset.Accession)))
	defer span.End()
	...
	if err := dmfh.triggerFileIngest(ctx, dodDataset.Accession, dodDataset.ObservationMetadata.MetadataFileType()); err != nil {
		...
		return fmt.Errorf("failed to trigger observation for image xml: %w", err)
	}
	if dodDataset.ObserverMetadata != nil {
		if err := dmfh.triggerFileIngest(ctx, dodDataset.Accession, dodDataset.ObserverMetadata.MetadataFileType()); err != nil {
			...
			return fmt.Errorf("failed to trigger observer for image xml: %w", err)
		}
	}
